...
//    CGContextSetStrokeColorWithColor(context, [UIColor blackColor].CGColor);
	...
	for (int i = 1; i<[_array count]; i++)
	{
        CGPoint goPoint;
        if (i == 1) {
            CGContextMoveToPoint(context1, 30, self.bounds.size.height - 30 - [[_array objectAtIndex:0] floatValue]*self.frame.size.height/160);
            goPoint = CGPointMake(30, self.bounds.size.height - 30 - [[_array objectAtIndex:0] floatValue]*self.frame.size.height/160);
        }else if (i < 5)
        {
            goPoint = CGPointMake(i*7+30, self.bounds.size.height - 30 - [[_array objectAtIndex:i-1] floatValue]*self.frame.size.height/160);
            CGContextAddLineToPoint(context1, goPoint.x, goPoint.y);;
        }else{
            goPoint = CGPointMake(i*11+8, self.bounds.size.height - 30 - [[_array objectAtIndex:i-1] floatValue]*self.frame.size.height/160);
            CGContextAddLineToPoint(context1, goPoint.x, goPoint.y);;
        }
