...
- (void)setButton
{
    ...
    UIButton *leftbutton = [[UIButton alloc] initWithFrame:CGRectMake(80, 12.5, 20, 20)];
    leftbutton.backgroundColor = [UIColor redColor];
    [leftbutton addTarget:self action:@selector(ClickLastButton) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:leftbutton];
    
    UIButton *rightbutton = [[UIButton alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width - 100, 12.5, 20, 20)];
    rightbutton.backgroundColor = [UIColor redColor];
    [rightbutton addTarget:self action:@selector(ClickNextButton) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:rightbutton];
}

- (void)ClickNextButton
{
//    [self.calendar nextMonth:_mydate animate:YES];
    _isNext = YES;
    _isLast = NO;
    [self.calendar moveMonth:_mydate animate:YES];
}
...
- (void)ClickLastButton
{
    ...
    _mydate = [calendar dateByAddingComponents:adcomps toDate:_mydate options:0];
//    [self.calendar lastMonth:_mydate animate:YES];
    [self.calendar moveMonth:_mydate animate:YES];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_scrollDirection == UICollectionViewScrollDirectionHorizontal) {
        ...
        if ((indexPath.item == 0 || indexPath.item == [collectionView numberOfItemsInSection:0] - 1 )) {
            ...
            if(_isNext)
            {
                _mydate = [_dateFormatter dateFromString:cell.titleLabel.text];
            }else if(_isLast)
            {
                _mydate = [_dateFormatter dateFromString:cell.titleLabel.text];
                NSDateComponents *adcomps = [[NSDateComponents alloc] init];
